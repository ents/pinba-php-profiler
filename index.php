...
class db {
	...
	protected function __construct(){
		...
		//$this->pdo = new PDO("mysql:dbname=pinba_cache;host=192.168.1.205;port=3306;encoding=utf8", "pinba_user", "pinbarulez");
...
{
	file_put_contents("file.log", count($treeList)."trees got to merge\n", FILE_APPEND);
	if (count($treeList) > 5) {
		...
		file_put_contents("file.log", "Devided into ".count($halfTree1)." and ".count($halfTree2)."\n", FILE_APPEND);
		return mergeTrees(array($halfTree1, $halfTree2));
	}
	file_put_contents("file.log", "Merging list...\n", FILE_APPEND);
	$result = array();
...
if (!empty($script_name)) {
	...
	if (file_exists($filename)) {
		...
		file_put_contents("file.log", "roots fetched\n", FILE_APPEND);
		$treeList = array();
		...
		file_put_contents("file.log", "trees builded\n", FILE_APPEND);

		$tree = mergeTrees($treeList);
		file_put_contents("file.log", "trees merged\n");
